...
Verification status can be checked by using `is_verified()` function.
...
is_verified(data_with_data) # is unverified
data_with_data <- within(data_with_data, i$rand <- sample(nrow(i)))
is_verified(data_with_data) # remains unverified
```
...
is_verified(data_right) # FALSE
data_right_verified <- verify(data_right) # returns verified object
is_verified(data_right_verified) # TRUE
```
