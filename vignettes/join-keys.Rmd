...
Refer to the section ["`ADaM` `join_keys`"](#adam-join-keys) for details on using this object to select specific datasets.
...
- `join_key(...)` specifies the relationship between two datasets:
...
Modifying or adding a key uses the same notation with the assignment operator `<-`. This will ensure symmetry as it will create both relationships. Assigning `NULL` value will delete the relationship.
...
`teal.data` provides a set of default join keys (primary and foreign) for datasets named according to the _ADaM_ standard. They are stored in `default_cdisc_join_keys`.

```{r}
names(default_cdisc_join_keys) |> sort()
```

If not all default keys are required, a subset can be used.
