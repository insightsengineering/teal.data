...
Alternatively, the primary keys can be defined in the `join_keys` parameter by using the `primary_key` or `join_key` functions _(note: when using `join_key` to define a primary key the second dataset name needs to be omitted or have the same value as the first)_.
...
data <- teal_data(
  ...
  join_keys = join_keys(
    primary_key("D1", c("X")),
    primary_key("D2", c("V", "W"))
    # join_key("D1", c("X"))             # equivalent to using primary_key
    # join_key("D2", "D2", c("V", "W")), # equivalent to using primary_key
  )
)
data$get_join_keys()
```
