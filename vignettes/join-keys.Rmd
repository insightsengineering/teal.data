...
Usually, an application developer needs to specify the keys manually, but in the case of datasets named according to the _ADaM_ standard, `teal.data` can assign the keys on its own via `default_cdisc_join_keys` object.
See the section called _["ADaM join_keys"](#adam-join_keys)_ below on how to use this object to select specific datasets.

##### Uses of `join_keys` in `teal` applications
...
## Accessing and adding/changing keys
