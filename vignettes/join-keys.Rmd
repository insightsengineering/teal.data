...
td <- teal_data(
  ds1 = ds1,
  code = "ds1 <- iris %>% mutate(id = factor(row_number()))",
  join_keys = join_keys(join_key("ds1", keys = "id"))

  ## Alternatively, when only a single primary key is defined the `join_keys()`
  ## wrapper can be skipped
  # join_keys = join_key("ds1", keys = "id")
)
...
data <- teal_data(
  ...
  join_keys = join_keys(
    ...
    # join_key("ds2", "ds2", keys = c("V", "W"))
  ),
  code = c(
    "ds1 <- data.frame(X = factor(1:10), Y = 21:30, Z = 1:10)",
    "ds2 <- data.frame(W = factor(10:1), V = factor(5:14), M = rep(1:5, 2))"
  )
)
...
data <- teal_data(
  ...
  code = c(
    "ds1 <- data.frame(X = factor(1:10), Y = 21:30, Z = 1:10)",
    "ds2 <- data.frame(W = factor(10:1), V = factor(5:14), M = rep(1:5, 2))",
    "ds3 <- data.frame(V = factor(5:14), T = 4)"
  )
)
...
```{r}
jk <- join_keys(
  join_key("ds1", keys = "col_1"),
  join_key("ds2", keys = c("col_1", "col_2")),
  join_key("ds3", keys = c("col_1", "col_3")),
  join_key("ds1", "ds2", keys = "col_1"),
  join_key("ds1", "ds3", keys = "col_1")
)
