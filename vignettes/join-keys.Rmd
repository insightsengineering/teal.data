...
Typically, an application developer manually specifies these keys. However, for datasets following the `ADaM` standard, `teal.data` can automatically assign keys using the `default_cdisc_join_keys` object. Refer to the section ["`ADaM` `join_keys`"](#adam-join_keys) for details on using this object to select specific datasets.

### Anatomy of Join Keys

A new `join_keys` can be created by defining a set of primary and foreign keys. The `join_keys` object can also be extended by establishing parent-child relationships between datasets and adding/modifying/removing keys.
