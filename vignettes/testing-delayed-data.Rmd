...
Once your delayed data object has been created as described in [Delayed Data Objects](delayed-data-objects.html), `teal.data` provides a useful set of functions to examine the object outside of a shiny application, i.e. the global environment. Below is an exhaustive list of all such functions:
...
There is also a `pull` method to test that the data can be loaded without launching a shiny app. See [Delayed Data Loading](delayed-data-loading.html).
...
See the section on [pre-processing Delayed Data](pre-processing-delayed-data.html) to specify additional code instructions to transform your delayed data which will also be added to the output of `get_code`.
