...
Data plays an essential role in data analysis and this package provide several ways to
...
as login, password, file location etc. before being loaded. Delayed data loading in applications
...
In standard delayed data loading application, before the main app loads a data loading screen is visible first.
This means that application can be two-staged:
...
`$set_args(list(ADSL = as.name(ADSL)))` to link object with function argument.
...
below is not allowed to make any changes in `ADSL`.
...
the code chunk below, a connector is created basing on `synthetic_cdisc_dataset`
function.
...
Data can be loaded using `load_dataset()` function. Before data loading, `adsl_conn` contains reproducible
...
The name `dummy_name` can be free to be any valid `R` name.
...
For example in a simplified version of `rice_connection`
