...
If you run the app above, the show R code button will return R code that contains big letters saying "Preprocessing is empty". In order to show the preprocessing code correctly the `code` argument of the `cdisc_data` function needs to be specified. For the above example this would be:
...
Providing the code in a copy-paste style as we did above is cumbersome and can lead to an out-of-sync situation where the code does not represent the preprocessing code anymore. We therefore provide the `get_code` function to extract the preprocessing code from the `app.R` file. The `get_code` function requires `#` tags to indicate which lines of code in `app.R` need be included in the preprocessing code. `get_code` understands the following tags:
...
`cdisc_data` call break reproducibility and are not allowed, e.g. developers shouldn't do an operation
