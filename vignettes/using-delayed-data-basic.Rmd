...
* The `teal_data` function takes a set of **`TealDataConnector`**, **`TealDatasetConnector`** and / or `cdisc_datasets` (non-delayed datasets) to create the **`TealData`** object which is used to create `teal` applications.
...
delayed_data <- teal_data(adsl_adae, advs)
```
