...
# initialize object
fun <- callable_function(fun = example_cdisc_data)
...
```{r}
adsl_raw <- example_cdisc_data(dataname = "ADSL")
...
adsl_raw <- example_cdisc_data("ADSL") %>% head(3)

adsl_dataset <- cdisc_dataset(
  dataname = "ADSL",
  x = adsl_raw,
  code = "ADSL <- example_cdisc_data(\"ADSL\") %>% head(3)"
)
```
...
data <- cdisc_data(
  cdisc_dataset(
    dataname = "ADSL",
    example_cdisc_data(dataname = "ADSL"),
    code = "ADSL <- example_cdisc_data(dataname = \"ADSL\")"
  ),
  cdisc_dataset(
    dataname = "ADTTE",
    example_cdisc_data("ADTTE"),
    code = "ADTTE <- example_cdisc_data(dataname = \"ADTTE\")"
  )
)
```
...
pull_adtte <- function(ADTTE) ADTTE <- teal.data::rADTTE
adtte <- dataset_connector(dataname = "ADTTE",
                           pull_callable = callable_function(fun = pull_adtte) %>% 
                             set_args(list(ADTTE = as.name("ADTTE"))),
                           keys = get_cdisc_keys("ADTTE"),
                           label = "ADTTE connector")
...
pull_adrs <- function(ADRS, n) ADRS <- head(teal.data::rADRS, n)
adrs <- dataset_connector(dataname = "ADRS",
                          pull_callable = callable_function(fun = pull_adrs) %>% # nolint
                            set_args(list(ADRS = as.name("ADRS"))),
