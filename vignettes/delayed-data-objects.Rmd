...
## Basic understanding
Delayed data objects are `R` objects that contain instructions on how to acquire data. In practice, you can use
`DDL` objects to pass data into a `teal` application without actually downloading the data before launching
application.

### Key concepts
* A **`TealDatasetConnector`** is an object used to pull a single delayed data set into a `teal` app.
Connectors to pull data from proprietary data storage are not included in this package.
```{r}
library(teal.data)
library(scda)
# create delayed data using scda (could
# also set keys using cdisc_dataset_connector)
adlb <- scda_dataset_connector("ADLB", "adlb", keys = get_cdisc_keys("ADLB"))

# create delayed data from code see package help for other connectors
...
```
* A **`TealDataConnector`** is an object used to pull a set of delayed data sets into a `teal` app which
...
```
* The `cdisc_data` function takes a set of **`TealDataConnector`**, **`TealDatasetConnector`**
and `cdisc_datasets` (non-delayed datasets) to create the **`TealData`** object which is used to create `teal`
applications.
```{r}
...
The datasets passed into `teal.data` and `cdisc_data` are pulled in the order they are defined so if datasets depend on other
datasets being available they should be placed later in the argument list:
...
# launch method will not be able to load the data as adae is pulled
# first but it depends on adsl
cdisc_data(adsl_2, adsl)
```

Note that in this case, the `adsl` object, which is used as an argument to `adsl_2` object,
need to be defined before the call to `cdisc_data`.
...
1. Replace the `cdisc_dataset` functions with the appropriate `TealDatasetConnector`.
1. Add preprocessing code as described above and verify once again by running the newly created object's `$launch` method.
1. Include just a single module to verify that the teal app starts and that it loads all the expected data.
1. Add modules one by one, replacing all dataset calls with strings and iteratively verify that the app functions as
expected.
