file_path <- testthat::test_path("app_get_code/app.R")
file_path1 <- testthat::test_path("app_get_code/app_source1.R")
file_path2 <- testthat::test_path("app_get_code/app_source2.R")
file_path3 <- testthat::test_path("app_get_code/app_source3.R")
...
test_that("Reads code from starts_at to stops_at", {
  code_lines1 <- strsplit(get_code(files_path = file_path, exclude_comments = FALSE), "\n")[[1]]
  expect_identical(code_lines1[c(length(code_lines1))], c("# \"this is a comment\""))
})

test_that("Excludes commented, keep quoted comments", {
  code_lines3 <- strsplit(
    get_code(files_path = file_path, exclude_comments = TRUE, read_sources = FALSE), "\n"
  )[[1]]
...
testthat::test_that("get_code.TealDataAbstract returns identical code to TealDataAbstract$get_code", {
  ...
  testthat::expect_identical(get_code(rd, deparse = TRUE), rd@code(deparse = TRUE))
  testthat::expect_identical(get_code(rd, deparse = FALSE), rd@code(deparse = FALSE))
