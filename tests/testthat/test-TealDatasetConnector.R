...
testthat::test_that("csv_dataset_connector attritubes", {
  ...
  teal.data::formatters_var_labels(ADSL_ns) <- letters[1:4] # nolint
  temp_file_csv <- tempfile(fileext = ".csv")
  ...
  data <- (x %>% mutate_dataset("teal.data::formatters_var_labels(ADSL) <- letters[1:4]"))$get_raw_data()
...
testthat::test_that("fun_cdisc_dataset_connector", {
  my_data_1 <- function() {
    ...
    teal.data::formatters_var_labels(x) <- c("STUDYID", "USUBJID", "z", "zz", "NAs", "w", "ww")
    x
  }
